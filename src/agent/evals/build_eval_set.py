...
def _load_config(cli_merchant_id: str | None) -> EvalBuildConfig:
    ...
    return EvalBuildConfig(
        ...
        questions_bank_path=Path("src/agent/evals/profiles_question_bank.yaml"),
...
def _build_items(config: EvalBuildConfig) -> list[dict[str, object]]:
    """Construct 15 grounded evaluation items from deterministic SQL facts.
    ...
    with duckdb.connect(str(config.duckdb_path), read_only=True) as conn:
        ...
        # 2) Tx count this week
        tx_count_week = _query_one(
            conn,
            """
            SELECT COUNT(*)
            FROM transactions
        ...
        items.append(
            {
                ...
                "expected_intent_id": "tx_count_period",
                "chart_allowed": False,
                "proactive_expected": False,
                "expected_facts": {
                    "period": {"start": week_start.isoformat(), "end": week_end.isoformat()},
                    "tx_count": _as_int(tx_count_week),
                },
                "expected_evidence": {
                    "query_key": "tx_count_period_week",
                    "params": {"merchant_id": merchant_id, "start_date": week_start.isoformat(), "end_date": week_end.isoformat()},
                },
                "pass_fail_criteria": {
        ...
        # 13) Unsupported intent: inventory
        items.append(
            {
                "eval_id": "E13",
